...
unsigned long efuse_w_disable;
unsigned char efuse_ate_wr_disable;
unsigned char wifi_mac[6];
unsigned char mac_crc;
unsigned char chip_verson_repeat4;
unsigned char chip_ver_spiconfig_reserve[3];
unsigned char ck8m_freq[2];
unsigned char sdio_ref[2];
unsigned char spi_pad_config[5];
};
...
//************************************************
//Read ID file, make sure id file is right, get id
//************************************************
...
sprintf(id_file_read.filepath,"./Chip_ID_ESP32_%d.txt",id_file_read.get_site);
...
if(!inIDFile){
  ...
  exit(1);
    ...
    cout<<"Read info as below:"<<endl;
    int i;
    for(i=0;i<10;i++){
        printf("id_file_read.chip_id_array[%d] is %x\n",i,id_file_read.chip_id_array);
    }
...
//************************************************
//Rewrite id file after efuse burning done
//************************************************
...
cout<<"New id as below:"<<endl;

for(i=0;i<9;i++){
    outIDFile<<hex<<id_file_read.chip_id_array[i]<<endl;
    cout<<id_file_read.chip_id_array[i]<<endl;
}
outIDFile.close();
...
// for stdf record chip_id_value
    ...
    struct ate_efuse efuse_to_burn=efuse_read;
    struct id_file_info id_file_read;
...
CSC_SERIAL_BLOCK_BEGIN
   ...
   // 3,burn efuse
   // 4,update id file
   // 5.record burned chip id in stdf


// 1,read id file,get chip id info
id_file_read=read_chip_id(machine_site);

cout<<"read id as below:"<<endl;
int i;
for(i=0;i<10;i++){
...
efuse_to_burn.wifi_mac[0]=to_burn_id_l%256;
efuse_to_burn.wifi_mac[1]=(to_burn_id_l>>8)%256;
efuse_to_burn.wifi_mac[2]=(to_burn_id_l>>16)%256;

efuse_to_burn.wifi_mac[3]=to_burn_id_h%256;
efuse_to_burn.wifi_mac[4]=(to_burn_id_h>>8)%256;
efuse_to_burn.wifi_mac[5]=(to_burn_id_h>>16)%256;

efuse_to_burn.mac_crc=calcrc_bytes(efuse_to_burn.wifi_mac,6);

cout<<"To burn id:"<<to_burn_id_h<<to_burn_id_l<<endl;
printf("To burn efuse: %02x%02x%02x_%02x%02x%02x\n",efuse_to_burn.wifi_mac[5],efuse_to_burn.wifi_mac[4],efuse_to_burn.wifi_mac[3],efuse_to_burn.wifi_mac[2],efuse_to_burn.wifi_mac[1],efuse_to_burn.wifi_mac[0]);
...
// 5.record burned chip id in stdf
  ...
  chip_id_value[*cscIter]=to_burn_id_l;
